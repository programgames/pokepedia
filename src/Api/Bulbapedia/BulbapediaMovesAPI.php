...
class BulbapediaMovesAPI
{
    ...
    public function getTutorMoves(Pokemon $pokemon, string $generation)
    {
        $moves = $this->cache->get(
            ...
            function (ItemInterface $item) use ($pokemon, $generation) {
                return $this->getMovesByPokemonGenerationAndType(
                    $pokemon,
                    $generation,
        ...
        if (empty($moveNames)) {
            throw new EmptyMoveSetException(
                sprintf('Empty moveset for pokemon %s in gen %s', $pokemon->getPokemonIdentifier(), $generation)
            );
        }
    ...
    {
        $moves = $this->cache->get(
            sprintf('awikitext.%s,%s.%s', $pokemon->getId(), $generation, MoveSetHelper::LEVELING_UP_TYPE),
            function (ItemInterface $item) use ($pokemon, $generation) {
                return $this->getMovesByPokemonGenerationAndType(
                    $pokemon,
                    $generation,
        ...
        $moves = $this->moveSatanizer->checkAndSanitizeLevelingMoves($moves, $generation);
